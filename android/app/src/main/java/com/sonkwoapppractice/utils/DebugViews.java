...
public final class DebugViews {
  ...
  private static String logViewHierarchy(@NonNull final View root){
    ...
    while (!stack.empty()){
      ...
      @NonNull final View view = p.second;
      ...
      final String line = graphics + className + dumpProperties(resources, view);
      ...
      if(view instanceof ViewGroup){
        ...
        for (int i = viewGroup.getChildCount() - 1; i>=0;i--){
          stack.push(Pair.create(prefix + (isLastOnLevel ? "    " : "│   "), viewGroup.getChildAt(i)));
        }
      }
    }
