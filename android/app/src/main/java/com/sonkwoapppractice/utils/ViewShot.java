package com.sonkwoapppractice.utils;
...
public class ViewShot implements UIBlock {
  ...
  private void saveToRawFileOnDevice(@NonNull final View view) throws  IOException {
    final String uri = output.getCanonicalPath();
    final FileOutputStream fos = new FileOutputStream((output));
  ...
  @NonNull
  private List<View> getAllChildren(@NonNull final View view){
  ...
  private Matrix applyTransformations(final Canvas canvas, @NonNull final View root, @NonNull final View child ) {
    ...
    for(final View view : ms){
      canvas.save();

      // apply each view transformations, so each child will be affected by them
      final float dx = view.getLeft() + ((view !=child) ? view.getPaddingLeft() : 0) + view.getTranslationX();
      final float dy = view.getTop() + ((view != child) ? view.getPaddingTop() : 0) + view.getTranslationY();
      canvas.translate(dx, dy);
      canvas.rotate(view.getRotation(), view.getPivotX(), view.getPivotY());
      canvas.scale(view.getScaleX(), view.getScaleX());

      // compute the matrix just for any future use
      transform.postTranslate(dx, dy);
      transform.postRotate(view.getRotation(), view.getPivotX(), view.getPivotY());
      transform.postScale(view.getScaleX(), view.getScaleY());
    }
    return  transform;
  }
